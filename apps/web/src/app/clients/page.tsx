...
export default function ClientsPage() {
  ...
  async function load(query?: string) {
    ...
    const res = await fetch(url); const json = await res.json() as { items: Client[] };
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    const res = await fetch(`${API}/v1/clients`, {
      ...
      body: JSON.stringify({ name: name.trim(), tags: tags.trim() ? tags.split(',').map(t=>t.trim()) : undefined })
    });
    if (res.ok) { setName(''); setTags(''); await load(q); }
  }
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="card lg:col-span-2">
          <div className="card-header">
            <div className="text-sm font-semibold">Search clients</div>
            <p className="help mt-1">Type a name or prefix (e.g., “fu” → Fulanito).</p>
          </div>
          <div className="card-body">
            <form onSubmit={(e)=>{e.preventDefault(); load(q);}} className="flex flex-col gap-2 sm:flex-row">
            ...
            <div className="mt-5 divide-y divide-slate-200 rounded-md border border-slate-200">
              ...
              {items.length === 0 && <div className="p-4 help">No results.</div>}
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="card-body">
            <form onSubmit={onCreate} className="space-y-3">
              <div><label className="label">Client name</label><input className="input mt-1" value={name} onChange={(e)=>setName(e.target.value)} placeholder="Acme Corp" /></div>
              <div><label className="label">Tags <span className="help">(comma separated)</span></label><input className="input mt-1" value={tags} onChange={(e)=>setTags(e.target.value)} placeholder="vip, mexico" /></div>
